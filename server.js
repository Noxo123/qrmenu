...
app.get('/api/qr', requireAuth, async (req,res)=>{ const r=myRestaurant(req); const target=`${BASE_URL}/m/${r.slug}`; const data=await QRCode.toDataURL(target,{width:600,margin:2}); res.json({target,data}); res.json({target,data}); });
