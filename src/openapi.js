function buildOpenApi(baseUrl) {
  ...
  const commonResponses = {
    '401': json('Token absent, invalide ou révoqué', { $ref: '#/components/schemas/Error' }),
    '403': json('Accès refusé', { $ref: '#/components/schemas/Error' }),
    '404': json('Ressource introuvable', { $ref: '#/components/schemas/Error' }),
    '429': json('Quota mensuel ou rate limit atteint', { $ref: '#/components/schemas/Error' })
  };
  ...
  return {
    ...
    tags: [
      { name: 'Health' }, { name: 'Restaurants' }, { name: 'Menu' }, { name: 'Categories' }, { name: 'Products' }, { name: 'Search' }, { name: 'Usage' }
    ],
    components: {
      securitySchemes: {
        BearerAuth: { type: 'http', scheme: 'bearer', bearerFormat: 'API_TOKEN', description: 'Utilisez un token qm_tok_... créé depuis le dashboard. Les anciennes clés qm_live_ restent compatibles avec les endpoints existants.' }
      },
      schemas: {
        Error: error,
        ...
        CreateRestaurantResponse: { allOf: [{ $ref: '#/components/schemas/ApiResponse' }, { type: 'object', properties: { data: { type: 'object', properties: { restaurant: { $ref: '#/components/schemas/Restaurant' }, default_category_id: { type: 'integer' }, menu_url: { type: 'string', format: 'uri' } } } } }] }
      }
    },
    paths: {
      ...
      '/api/v1/usage': { get: { tags: ['Usage'], summary: 'Consulter le quota du token', security: [{ BearerAuth: [] }], responses: { '200': json('Quota et tokens', success), ...commonResponses } } },
      '/api/v1/restaurants': {
        get: { tags: ['Restaurants'], summary: 'Lister les restaurants du compte', security: [{ BearerAuth: [] }], responses: { '200': json('Liste des restaurants', success), ...commonResponses } },
        post: { tags: ['Restaurants'], summary: 'Créer un restaurant depuis l’API', security: [{ BearerAuth: [] }], requestBody: { required: true, content: { 'application/json': { schema: { $ref: '#/components/schemas/CreateRestaurant' }, example: { name: 'Chez Marius', slug: 'chez-marius', description: 'Cuisine maison', phone: '0612345678', address: '10 rue de Paris', accent_color: '#19a463' } } } }, responses: { '201': json('Restaurant créé', { $ref: '#/components/schemas/CreateRestaurantResponse' }), '402': json('Abonnement requis pour une fonctionnalité dépassant le plan', { $ref: '#/components/schemas/Error' }), '422': json('Données invalides', { $ref: '#/components/schemas/Error' }), ...commonResponses } }
      },
      ...
      '/api/v1/restaurants/{slug}': { get: { tags: ['Restaurants'], summary: 'Récupérer les informations d’un restaurant', security: [{ BearerAuth: [] }], parameters: [slug], responses: { '200': json('Restaurant', success), ...commonResponses } } },
